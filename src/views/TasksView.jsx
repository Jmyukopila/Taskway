    ...
    <div className="flex-1 px-4 pt-4 pb-24 overflow-y-auto">
      ...
      <div className="mb-3 animate-fade-in-up" style={{ animationDelay: '50ms' }}>
        <div className="relative">
          <SearchIcon className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 pointer-events-none" style={{ color: 'var(--color-muted)' }} />
          <input
            type="text"
          ...
          />
          {busqueda && (
            <button onClick={() => setBusqueda('')} className="absolute right-2.5 top-1/2 -translate-y-1/2 p-1" style={{ color: 'var(--color-muted)' }} aria-label="Limpiar busqueda">
          ...
          {tareasAgrupadas.map(([fecha, tareas], idx) => {
            ...
            return (
              <div key={fecha} className="animate-fade-in-up" style={{ animationDelay: `${idx * 60}ms` }}>
                <button onClick={() => toggleFecha(fecha)} className="flex items-center gap-2 w-full mb-2" aria-expanded={isExpanded}>
                  <ChevronRightIcon
